...
PASS  empty: no tool registrations  [{"__dynamicRegistrationHint":false,"__dynamicPackages":[]}]
...
PASS  dup-tool: same name scanned from both  [{"pkg-dup-a":["dup_tool"],"pkg-dup-b":["dup_tool"],"__dynamicRegistrationHint":false,"__dynamicPackages":[]}]
...
PASS  leak: bare setInterval -> leak-suspect  [apply 路径裸副作用未释放：setInterval —— apply 裸注册超出清理 x1]
